...
export default function WorldView() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (sats.length > 0) { satsRef.current = sats; setSatCount(sats.length); setSatStatus(`${sats.length} SATS`); }
      else setSatStatus("SAT FETCH FAILED");
    };
    load();
  ...
  // Fetch earthquakes
  useEffect(() => {
    const load = async () => { const q = await fetchEarthquakes(); quakesRef.current = q; setQuakeCount(q.length); };
    load();
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        ctx.fillStyle = `rgba(${F.star[0]},${F.star[1]},${F.star[2]},${0.15 + 0.2 * Math.sin(autoT * 0.5 + i)})`;
      ...
      g1.addColorStop(0.5, F.atmoColor);
      ...
      gO.addColorStop(0, F.ocean[0]); gO.addColorStop(1, F.ocean[1]);
      ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI * 2);
      ctx.fillStyle = gO; ctx.fill();
      ctx.strokeStyle = F.globeEdge; ctx.lineWidth = 1.5; ctx.stroke();

      // Graticule
      const graticule = d3.geoGraticule().step([15, 15]);
      ctx.beginPath(); path(graticule()); ctx.strokeStyle = F.graticule; ctx.lineWidth = 0.4; ctx.stroke();
      ...
      if (geo) {
        geo.features.forEach(f => {
          ...
          if (id === IRAN_CODE) {
            ctx.fillStyle = F.iranFill; ctx.fill();
            ctx.strokeStyle = F.iranStroke; ctx.lineWidth = 2; ctx.stroke();
          } else if (ME_CODES.includes(id)) {
            ctx.fillStyle = F.meLand; ctx.fill();
            ctx.strokeStyle = F.meStroke; ctx.lineWidth = 0.8; ctx.stroke();
          } else {
            ctx.fillStyle = F.land; ctx.fill();
            ctx.strokeStyle = F.landStroke; ctx.lineWidth = 0.5; ctx.stroke();
          }
        });
      }

      ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI * 2);
      ctx.strokeStyle = F.globeEdge; ctx.lineWidth = 1; ctx.stroke();

      // Day/Night Terminator
      if (layers.terminator) {
        try {
          const term = computeTerminator(Date.now());
          const nightGeo = { type: "Feature", geometry: { type: "Polygon", coordinates: [[ ...term.points.map(([lng, lat]) => [lng, lat]), [180, term.declination > 0 ? -90 : 90], [-180, term.declination > 0 ? -90 : 90] ]] } };
      ...
      // ── GPS Jamming Zones ──
      if (layers.gpsJam) {
        GPS_JAM_ZONES.forEach(zone => {
          const p = projection([zone.lng, zone.lat]);
          if (!p) return;
          const d = d3.geoDistance([zone.lng, zone.lat], [rLng, rLat]);
          if (d > Math.PI / 2) return;
          const col = jamColor(zone.severity);
          const opa = jamOpacity(zone.severity);
          // Radius in pixels (approximate: 1 degree ~ R * PI / 180)
          const degRadius = zone.radius / 111;
          const pxRadius = degRadius * R * Math.PI / 180;
          // Pulsing zone fill
          const pulseR = pxRadius * (0.9 + 0.1 * Math.sin(autoT * 2));
          ctx.beginPath(); ctx.arc(p[0], p[1], pulseR, 0, Math.PI * 2);
          ctx.fillStyle = col + Math.round(opa * 255).toString(16).padStart(2, "0"); ctx.fill();
          // Outer ring
          ctx.beginPath(); ctx.arc(p[0], p[1], pulseR, 0, Math.PI * 2);
          ctx.strokeStyle = col + "55"; ctx.lineWidth = 1; ctx.stroke();
          // Dashed outer boundary
          ctx.setLineDash([4, 4]);
          ctx.beginPath(); ctx.arc(p[0], p[1], pulseR * 1.2, 0, Math.PI * 2);
          ...
          if (Z > 0.7) {
            ...
            ctx.fillText(`GPS JAM`, p[0] - 18, p[1] - pulseR - 4);
            ctx.font = "6px monospace";
            ctx.fillStyle = col + "88";
            ctx.fillText(zone.name, p[0] - 18, p[1] - pulseR - 13);
          }
        });
      }
      ...
      CITIES.forEach(([lat, lng, name]) => {
        ...
        const d = d3.geoDistance([lng, lat], [rLng, rLat]); if (d > Math.PI / 2) return;
        ctx.beginPath(); ctx.arc(p[0], p[1], 2, 0, Math.PI * 2);
        ctx.fillStyle = F.cityColor; ctx.fill();
        if (Z > 0.8) { ctx.font = "8px monospace"; ctx.fillStyle = F.cityText; ctx.fillText(name, p[0] + 4, p[1] + 3); }
      });
      HOT_CITIES.forEach(([lat, lng, name]) => {
        const p = projection([lng, lat]); if (!p) return;
        const d = d3.geoDistance([lng, lat], [rLng, rLat]); if (d > Math.PI / 2) return;
        ctx.beginPath(); ctx.arc(p[0], p[1], 3.5, 0, Math.PI * 2);
        ctx.fillStyle = F.hotCity; ctx.fill();
        if (Z > 0.7) { ctx.font = "bold 9px monospace"; ctx.fillStyle = F.hotText; ctx.fillText(name, p[0] + 5, p[1] + 3); }
      });

      // Targets
      TARGETS.forEach(t => {
        const p = projection([t.lng, t.lat]); if (!p) return;
        const d = d3.geoDistance([t.lng, t.lat], [rLng, rLat]); if (d > Math.PI / 2) return;
      ...
      if (layers.flights) {
        flightsRef.current.forEach(f => {
          ...
          const d = d3.geoDistance([f.lng, f.lat], [rLng, rLat]); if (d > Math.PI / 2) return;
      ...
      if (layers.sats) {
          ...
          try {
            ...
            if (trace.length > 2) {
              ...
              trace.forEach(pt => {
                ...
                if (p && d < Math.PI / 2) { if (!started) { ctx.moveTo(p[0], p[1]); started = true; } else ctx.lineTo(p[0], p[1]); }
                else { started = false; }
              });
        ...
        sats.forEach(sat => {
          try {
            ...
            const p = projection([pos.lng, pos.lat]); if (!p) return;
            const d = d3.geoDistance([pos.lng, pos.lat], [rLng, rLat]); if (d > Math.PI / 2) return;
            ...
            if ((cls.isMil || cls.isRecon || cls.isISS) && Z > 0.7) {
              ctx.font = "bold 7px monospace"; ctx.fillStyle = cls.color + "99";
      ...
      if (layers.quakes) {
        quakesRef.current.forEach(q => {
          const p = projection([q.lng, q.lat]); if (!p) return;
          const d = d3.geoDistance([q.lng, q.lat], [rLng, rLat]); if (d > Math.PI / 2) return;
          const sz = quakeSize(q.mag); const col = quakeColor(q.mag);
          const pulse = sz + 3 * Math.sin(autoT * 3);
          ctx.beginPath(); ctx.arc(p[0], p[1], pulse, 0, Math.PI * 2);
          ctx.strokeStyle = col + "55"; ctx.lineWidth = 1; ctx.stroke();
          ctx.beginPath(); ctx.arc(p[0], p[1], sz, 0, Math.PI * 2);
          ctx.fillStyle = col; ctx.fill();
          if (q.mag >= 4 && Z > 0.7) { ctx.font = "bold 7px monospace"; ctx.fillStyle = col + "bb"; ctx.fillText(`M${q.mag.toFixed(1)}`, p[0] + sz + 3, p[1] + 2); }
        });
      }
      ...
      if (layers.ships) {
        shipsRef.current.forEach(s => {
          const p = projection([s.lng, s.lat]); if (!p) return;
          const d = d3.geoDistance([s.lng, s.lat], [rLng, rLat]); if (d > Math.PI / 2) return;
          const col = shipColor(s.type); const sz = s.type === "NAVAL" ? 3.5 : 2.5;
          ctx.beginPath(); ctx.moveTo(p[0], p[1]-sz); ctx.lineTo(p[0]+sz*0.7, p[1]); ctx.lineTo(p[0], p[1]+sz); ctx.lineTo(p[0]-sz*0.7, p[1]); ctx.closePath();
          ctx.fillStyle = col; ctx.fill();
          if (s.hdg) { const h = s.hdg * Math.PI / 180; const ep = projection([s.lng + Math.sin(h)*0.8, s.lat + Math.cos(h)*0.8]); if (ep) { ctx.beginPath(); ctx.moveTo(p[0],p[1]); ctx.lineTo(ep[0],ep[1]); ctx.strokeStyle = col+"66"; ctx.lineWidth = 0.8; ctx.stroke(); } }
          if (s.type === "NAVAL" && Z > 0.9) { ctx.font = "bold 7px monospace"; ctx.fillStyle = col+"99"; ctx.fillText(s.name, p[0]+5, p[1]+2); }
        });
      }

      // Light reflection + crosshair
      const g2 = ctx.createRadialGradient(cx - R*0.3, cy - R*0.3, 0, cx, cy, R);
      g2.addColorStop(0, F.reflection); g2.addColorStop(1, "transparent");
      ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI * 2); ctx.fillStyle = g2; ctx.fill();
      ctx.strokeStyle = F.crosshair; ctx.lineWidth = 0.8;
      [[cx,cy-22,cx,cy-8],[cx,cy+8,cx,cy+22],[cx-22,cy,cx-8,cy],[cx+8,cy,cx+22,cy]].forEach(([x1,y1,x2,y2]) => { ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke(); });
    };
    render();
    return () => { cancelAnimationFrame(animId); };
  }, [mapLoaded, layers, visualFilter]);

  // ── Globe interaction handlers ──
  const lastDistRef = useRef(0);
  const handleDown = (e) => {
    const touch = e.touches?.[0];
    dragRef.current.dragging = true; dragRef.current.moved = false;
    dragRef.current.lx = touch?.clientX ?? e.clientX; dragRef.current.ly = touch?.clientY ?? e.clientY;
    dragRef.current.startX = dragRef.current.lx; dragRef.current.startY = dragRef.current.ly;
    if (e.touches?.length === 2) { const dx = e.touches[0].clientX - e.touches[1].clientX; const dy = e.touches[0].clientY - e.touches[1].clientY; lastDistRef.current = Math.sqrt(dx*dx+dy*dy); }
  };
  const handleUp = (e) => {
    ...
    if (!wasDrag && projRef.current) {
      ...
      const proj2 = d3.geoOrthographic().scale(projRef.current.R).translate([projRef.current.cx, projRef.current.cy]).rotate([-rLng, -rLat, 0]).clipAngle(90);
  ...
  const handleMove = (e) => {
    ...
    const touch = e.touches?.[0]; const x = touch?.clientX ?? e.clientX; const y = touch?.clientY ?? e.clientY;
    if (Math.abs(x-dragRef.current.startX)>5 || Math.abs(y-dragRef.current.startY)>5) { dragRef.current.moved = true; setSelected(null); }
    ...
    if (e.touches?.length === 2) { const tdx = e.touches[0].clientX-e.touches[1].clientX; const tdy = e.touches[0].clientY-e.touches[1].clientY; const dist = Math.sqrt(tdx*tdx+tdy*tdy); if (lastDistRef.current>0) zoomRef.current = Math.max(0.5,Math.min(3,zoomRef.current*(dist/lastDistRef.current))); lastDistRef.current = dist; }
  };
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: F.bg, position: "relative", overflow: "hidden", fontFamily: "'Courier New',monospace", userSelect: "none" }}>
      ...
      {/* LAYER TOGGLES + FILTER - top right */}
      <div style={{ position: "absolute", top: 50, right: 8, zIndex: 10, display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        <div style={{ marginTop: 4, borderTop: "1px solid #1a3a5c33", paddingTop: 4 }} />
      ...
      {/* DATA PANEL */}
      {panel === "data" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px" }}>
          <div style={{ color: F.color, fontSize: 14, fontWeight: "bold", marginBottom: 10 }}>LIVE INTELLIGENCE</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 20px" }}>
            {[["AIRBORNE",hud.planes.toLocaleString(),"#00ccff"],["MIL SUSPECT",hud.mil,"#ff4444"],["AVG ALT",hud.avg.toLocaleString()+"m","#00ffaa"],["SATELLITES",satCount||"...","#aa44ff"],["EARTHQUAKES",quakeCount||"...","#ffaa00"],["SHIPS",shipCount||"...","#44ddaa"],["GPS JAM ZONES",GPS_JAM_ZONES.length,"#ff6600"],["FILTER",F.label,F.color]].map(([l,v,c],i) => (
      ...
      {/* TARGETS PANEL */}
      {panel === "targets" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px", maxHeight: "40vh", overflowY: "auto" }}>
          {TARGETS.map((z, i) => (
      ...
      {/* SATS PANEL */}
      {panel === "sats" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px", maxHeight: "45vh", overflowY: "auto" }}>
          ...
          {satsRef.current.slice(0, 30).map((s, i) => { const cls = classifySat(s.OBJECT_NAME); return (
            <div key={i} style={{ padding: "4px 0", borderBottom: "1px solid #1a3a5c15", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 7, height: 7, borderRadius: "50%", background: cls.color, flexShrink: 0 }} /><div><div style={{ color: cls.color, fontSize: 11, fontWeight: "bold" }}>{s.OBJECT_NAME}</div><div style={{ color: "#1a3a5c", fontSize: 9 }}>NORAD {s.NORAD_CAT_ID} - {Math.round(1440/s.MEAN_MOTION)}min</div></div></div>
      ...
      {panel === "quakes" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px", maxHeight: "45vh", overflowY: "auto" }}>
          ...
          {quakesRef.current.slice(0, 25).map((q, i) => (
            <div key={i} style={{ padding: "4px 0", borderBottom: "1px solid #1a3a5c15", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><div style={{ color: quakeColor(q.mag), fontSize: 11, fontWeight: "bold" }}>M{q.mag.toFixed(1)} - {q.place}</div><div style={{ color: "#1a3a5c", fontSize: 9 }}>Depth: {q.depth?.toFixed(1)}km - {new Date(q.time).toLocaleTimeString()}</div></div>
      ...
      {panel === "ships" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px", maxHeight: "45vh", overflowY: "auto" }}>
          ...
          <div style={{ color: "#1a3a5c", fontSize: 10, marginBottom: 10 }}>Maritime traffic - AIS data</div>
          {shipsRef.current.map((s, i) => (
            <div key={i} style={{ padding: "4px 0", borderBottom: "1px solid #1a3a5c15", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 7, height: 7, background: shipColor(s.type), flexShrink: 0, transform: "rotate(45deg)" }} /><div><div style={{ color: shipColor(s.type), fontSize: 11, fontWeight: "bold" }}>{s.name}</div><div style={{ color: "#1a3a5c", fontSize: 9 }}>{s.callsign} - {s.flag} - {s.length}m</div></div></div>
      ...
      {panel === "gpsJam" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px", maxHeight: "45vh", overflowY: "auto" }}>
          <div style={{ color: "#ff6600", fontSize: 14, fontWeight: "bold", marginBottom: 4 }}>GPS/GNSS INTERFERENCE ZONES</div>
          <div style={{ color: "#1a3a5c", fontSize: 10, marginBottom: 10 }}>Based on EUROCONTROL advisories and OSINT reports</div>
          {GPS_JAM_ZONES.map((z, i) => (
            <div key={i} style={{ padding: "5px 0", borderBottom: "1px solid #1a3a5c15", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ color: jamColor(z.severity), fontSize: 12, fontWeight: "bold" }}>{z.name}</div>
                <div style={{ color: "#1a3a5c", fontSize: 9 }}>{z.desc}</div>
              </div>
              <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 8 }}>
                <div style={{ color: jamColor(z.severity), fontSize: 11, fontWeight: "bold" }}>{z.severity}</div>
                <div style={{ color: "#1a3a5c", fontSize: 9 }}>{z.radius}km / {z.source}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {panel === "legend" && (
        <div className="panel-overlay" style={{ position: "fixed", bottom: 85, left: 0, right: 0, zIndex: 25, background: F.bg + "ee", borderTop: "1px solid #1a3a5c", padding: "14px 16px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 14px" }}>
            {[["#00ccff","Commercial flight"],["#ff4444","Mil suspect / Mil sat"],["#ff2200","Strike zone"],["#ffaa00","Nuclear / Recon / Quake"],["#00ff88","Airbase / ISS"],["#aa44ff","Satellite orbit"],["#44ddaa","Ship / Vessel"],["#ff6600","GPS Jamming zone"],["rgba(255,180,50,0.5)","Day/Night terminator"]].map(([c,l],i) => (
          ...
          <div style={{ color: "#1a3a5c", fontSize: 11, marginTop: 10 }}>OpenSky + CelesTrak + USGS live | Drag rotate | Pinch zoom | Tap for info</div>
          <div style={{ color: "#1a3a5c", fontSize: 10, marginTop: 4 }}>Visual filters: DEFAULT / NVG / FLIR / SIGINT (top-right button)</div>
        </div>
      )}
    </div>
  );
